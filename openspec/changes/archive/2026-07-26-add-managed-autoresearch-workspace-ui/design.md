...
The versioned `/api/v1` API is unauthenticated and available only through the loopback listener. The service rejects non-loopback hosts, disables permissive CORS, validates `Host` and mutation `Origin`, and emits a restrictive content security policy.
...
- [A local control API can become arbitrary remote execution] -> Bind to loopback, validate host and mutation origin, allowlist operations and arguments, confine paths, and never accept shell command text.
