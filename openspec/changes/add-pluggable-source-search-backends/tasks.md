...
- [ ] 1.1 Add schema-version-2 indexing configuration with an ordered approved-backend list and closed capability routes.
  ...
  - Состояние шага: целевая реализация и профильные Python, frontend и browser проверки завершены; выполняется детерминированная синхронизация и проверка упаковочной границы
  - Следующее действие: повторно синхронизировать runtime и scaffold из очищенной target-ссылки, пересобрать артефакты и проверить отсутствие операционных данных
  - Файлы шага: openspec/changes/add-pluggable-source-search-backends/tasks.md, scripts/sync_generated_runtime.py, templates/research-repo, src/one_c_autoresearch, web/workspace, tests
