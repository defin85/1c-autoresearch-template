...
- Run `python -m one_c_autoresearch doctor` before updating the selected task status.
...
3. Atomically claim one pending task with the highest priority whose dependencies are done:
...
8. Run `python -m one_c_autoresearch doctor` and report any warnings or failures.
9. Update the selected task to `evidence_pack`, `drafted`, `needs_review`, `needs_followup`, `blocked`, or `done`. Use `--expected-status claimed` when moving a freshly claimed task forward.
