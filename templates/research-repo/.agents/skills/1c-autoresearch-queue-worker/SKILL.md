...
- Run `scripts\doctor.ps1` before updating the selected task status.
...
8. Run `scripts\doctor.ps1` and report any warnings or failures.
9. Update the selected task to `evidence_pack`, `drafted`, `needs_review`, `needs_followup`, `blocked`, or `done`. Use `-ExpectedStatus claimed` when moving a freshly claimed task forward.
