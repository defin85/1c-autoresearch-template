...
FILES = ("AGENTS.md", "README.md", "pyproject.toml", "uv.lock", "research/workflow.toml", "research/indexing.toml", "research/forbidden-authorities.json", "tests/test_external_folder.py")
TREES = ("src/one_c_autoresearch", "one_c_autoresearch", "research/schemas", "web/workspace")
...
def write_seed(root: Path) -> None:
    ...
    lines = ['schema_version = "1"', 'acquisition_profile = "ibcmd+xml-hierarchical/v1"', ""]
...
def sanitize(root: Path, reference: Path) -> None:
    ...
    for path in root.rglob("*"):
        ...
        text = text.replace("# SPPR Research", "# __PRODUCT__ Research").replace("SPPR", "Example").replace("sppr-research", "__PROJECT_ID__")
